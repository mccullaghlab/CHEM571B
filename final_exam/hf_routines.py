...
def perform_hf_scf(basis,atoms,N,maxIter=20,thresh=1E-4):
    ...
    deltaE = 1.0
    iteration = 0 
    while deltaE > thresh and iteration < maxIter:
        P = constructDensityMat(C,N)
        G = compute_G(P,twoE)
        F = Hcore + G
        hf_energy, C, orbitalEnergies = total_energy(F,X,Hcore,P,atoms,N)
        deltaE = abs(hf_energy-previousE)
        previousE = hf_energy
        iteration += 1
    
    hf_twoE = update_twoE(C,twoE)
    return hf_energy, C, Hcore, F, hf_twoE
